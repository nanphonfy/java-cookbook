package cn.sibat.boot.server.util.common;
...
public class RegValidationUtil {
    ...
    /**
     * https://zhidao.baidu.com/question/185682526.html
     * 汉字在unicode中的编码不是完全连续的，常用的范围是\u4e00-\u9fa5，\u3447-\uFA29]可能是包含所有汉字的编码范围，但是其中必然也会包含其他字符。
     */
    ...
    /**
     * 身份证为什么有15位
     * https://zhidao.baidu.com/question/687897498385925844.html
     */
     ...
     */
    public static boolean validateStrLength(int lowerLength, int upperLength, String input)
            throws UnsupportedEncodingException {
        if (StringUtils.isEmpty(input)) {
            return false;
        }
        byte[] inputStr = input.getBytes("utf-8");
        if (inputStr.length < lowerLength) {
            return false;
        }
        if (inputStr.length > upperLength) {
            return false;
        }
        return true;
    }
