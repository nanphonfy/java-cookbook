...
public class DataFormatUtilInterfaceImpl implements DataFormatUtilInterface {
     ...
     * 传入不可转换字符 抛出异常
     */
    @Override public double formatToDouble(Object value, Integer column) {
        if (value == null) {
            return 0;
        }
     ...
     * 传入不可转换字符 抛出异常
     */
    @Override public int formatToInt(Object value, Integer column) {
        if (value == null) {
            return 0;
        }
        String data = String.valueOf(value);
     ...
     */
    @Override public Date format(Object timestemp, Integer column) {
        if (timestemp == null) {
            return null;
        }
    ...
    @Override public String formatToDateString(String timestemp, String pattern, Integer column) {
        if (timestemp == null) {
            return null;
        }
    ...
    @Override public String formatToDateString(Long timestemp, String pattern, Integer column) {
        if (timestemp == null) {
            return null;
        }
        SimpleDateFormat sd = getFormat(pattern);
    ...
    public static SimpleDateFormat getFormat(String pattern) {
        if (pattern == null) {
            throw new ExcelDataFormatException("pattern 不能为空");
        }
        return new SimpleDateFormat(pattern);
    }

    @Override public long formatToLong(Object value, Integer column) {
        if (value == null) {
            return 0;
        }
        if (value instanceof BigDecimal) {
    ...
    @Override public double formatToDouble(Object value) {

        return formatToDouble(value, null);
    }

    @Override public int formatToInt(Object value) {

        return formatToInt(value, null);
    }

    @Override public long formatToLong(Object value) {
        return formatToLong(value, null);
    }

    @Override public Date format(Object timestemp) {
        return format(timestemp, null);
    }

    @Override public String formatToDateString(String timestemp, String patten) {
        return formatToDateString(timestemp, patten, null);
    }

    @Override public String formatToDateString(Long timestemp, String patten) {
        return formatToDateString(timestemp, patten, null);
    }
}
